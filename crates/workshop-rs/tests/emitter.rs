...
fn rule_final_if_omits_the_trailing_end_and_round_trips() {
    // A rule-final if/if-else closes with the `}` (no trailing `End;`),
    // byte-equal to the oracle's spelling; the ws parser accepts it and
    // re-emission is a byte-identical fixed point. The middle-of-rule if
    // keeps `End;`.
...
fn constant_format_calls_fold_to_the_substituted_text() {
    // All-constant `.format()` calls fold into the substituted Custom String
    // text (oracle spelling), including the two-decimal float rendering;
    // variable arguments stay as format nodes.
    let mut program = wir::Program::default();
    let file = program
        .files
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let text = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("value: {0}".to_string()),
        None,
    ));
    let three = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Number {
            value: 3.0,
            text: "3".to_string(),
        },
    ...
    let folded = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Call {
...
fn constant_float_format_arguments_use_two_decimals() {
    ...
    let file = program
        ...
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let text = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("v: {0}".to_string()),
        None,
    ));
    let half = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Number {
            value: 0.5,
            text: "0.5".to_string(),
        },
...
fn split_and_reescaped_value_strings_round_trip_byte_identically() {
    // A long string (split continuation chain) and an escaped string parse
    // and re-emit byte-identically through the ws parser.
    let mut program = wir::Program::default();
    let file = program
        .files
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let long = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("B".repeat(300)),
        None,
    ));
    let escaped = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("a\nb\"c\\d\te".to_string()),
        None,
    ));
...
fn implicit_format_placeholders_renumber_to_the_oracle_form() {
    // Implicit `{}` placeholders renumber positionally to the oracle's
    // explicit form (`"v: {}".format(x)` -> `Custom String("v: {0}",
    // Global.x)`), and constant arguments fold into the text with the
    // remaining placeholders renumbered.
    let mut program = wir::Program::default();
    let file = program
        .files
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let text = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("v: {}".to_string()),
        None,
    ));
...
fn partial_constant_format_folds_and_renumbers() {
    ...
    let file = program
        ...
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let text = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::String("{} {}".to_string()),
        None,
    ));
    let three = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Number {
...
fn playervar_reads_parenthesize_the_receiver() {
    // `g = eventPlayer.p` emits `Set Global Variable(g, (Event Player).p)`
    // (byte-quoted oracle pin) and the spelling round-trips through the ws
    // parser.
    let mut program = wir::Program::default();
    let file = program
        .files
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    let player = program.values.push(workshop_rs::wir::ValueNode::new(
...
fn receiver_call_actions_and_values_emit_catalog_spellings() {
    // `.opy` receiver calls lower to `Action::Call`/`Value::Call` whose
    // `name` is the receiver method; emission resolves those names through
    // the catalog (general path, no per-name special cases).
    let mut program = wir::Program::default();
    program
        .files
        .push(workshop_rs::source::SourceFile::new("workshop.txt"));
    ...
    let percent = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Number {
            value: 100.0,
            text: "100".to_string(),
        },
    ...
    let alive = program.values.push(workshop_rs::wir::ValueNode::new(
        workshop_rs::wir::Value::Call {
