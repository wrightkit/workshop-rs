...
| First-listed `zh-CN` spelling of ten actions (`destroyAllIcons`, `destroyAllInWorldText`, `destroyAllEffects`, `destroyAllHudText`, `disableBuiltinGameMode{Announcer,Completion,Music,Scoring}`, `enableBuiltinGameModeMusic`, `unpauseMatchTime`) | Pinned OverPy 9.7.10 zh-CN emission of the corresponding functions, per [ADR-0014](../adr/0014-validation-evidence-for-slot-acceptance.md); the previously emitted spelling stays a parse alias. See [wrapper forms](../wrapper-forms.md). |
