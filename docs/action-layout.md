...
action lines produced by a public `Action` sequence. The result includes structural
headers and terminators, so a structured action can have a width greater than
one.

The query validates the canonical program first and then expands the requested
...
Malformed programs and emission failures are returned as
`ActionLayoutError::InvalidProgram` or `ActionLayoutError::Emission`; they are
