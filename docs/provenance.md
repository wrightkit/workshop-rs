...
| Localized parameter labels (`paramAliases`) | Owner-reviewed catalog locale mappings, stored per declared parameter position and resolved only through the catalog API; the initial `Wait` `zh-CN` labels are committed as the representative mapping. |
