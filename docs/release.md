...
`workshop-rs` library against the accepted `0.3.9` release baseline. The
protected surface is the documented public library API established by #112.
...
Semver-safe additive changes do not require baseline updates. An intentional
breaking change must first have an approved public-contract decision, use the
corresponding major version release, and update the CI baseline to the last
accepted published release in the same change.
